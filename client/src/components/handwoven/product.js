...
const Product = ({ image, title, price,id }) => {
  ...
  return (
    <div
      className={`handwoven-card ${hover ? 'hover' : ''}`}
    ...
    >
      ...
      <div className="handwoventitleContainer">
        <h3>{title}</h3>
        <div className="handwovenlikeImgContainer">
          <img src={like} className='handwovenlikeImg'onClick={()=>{
        handlewishlistclick(id)
      }} />
        </div>
      </div>
      <p>₹ {price}</p>
      <button className="handwovenadd-to-cart-btn" onClick={()=>{
        handlecartclick(id)
      }}>ADD TO CART</button>
    </div>
  );
